...
export const buildSectionData = ({ content, sectionOrder, sectionVisibility }) => {
  const sections = {};
  ...
  if (sectionVisibility?.socialLinks !== false) {
    ...
    if (entries.length) sections.socialLinks = { type: 'socialLinks', data: entries };
  }

  const orderedIds = (sectionOrder || Object.keys(sections)).filter((id) => sections[id]);
  return { sections, orderedIds };
};

export const getResumeStyles = (settings = {}) => {
  ...
  return { color, accent, fontFamily, fontSize };
};
